...
class ListModule extends Component {
    ...
    checkBoxItems = (modules, items) => {
        let {ListModules} = this.state
        console.log('=====modules:', modules)
        return modules && modules.length > 0 && modules.map((item, i) => {
            let refdata = {
                type: 'module',
    ...
    setModuleCheckedList = () => {
        let {userPackageModuleActs, items, modules} = this.props
    ...
    render(){
        ...
        return(
            <Grid>
                <Grid.Row>
                    {this.checkBoxItems(modules, this.props.items)}
                </Grid.Row>
            </Grid>
        )
    }
