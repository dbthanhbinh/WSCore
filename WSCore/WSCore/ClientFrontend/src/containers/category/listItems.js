...
import { checkHasPermission } from '../../utilities/cookies'
...
const ListItems = (props) => {
    ...
    const renderLinkItem = (id) => {
        let hasPermission = checkHasPermission(userModuleActs, actions.EDIT)
        return (id === currentId || !hasPermission) ? <span>Edit</span> : <Link to={`/${controlled?.toLowerCase()}/${actions.EDIT?.toLowerCase()}/${id}`}>Edit</Link>
    }

    const renderRemoveItem = (id) => {
        let hasPermission = checkHasPermission(userModuleActs, actions.EDIT)
        return (id === currentId ) ? <span><Icon name='remove' /></span> : <span onClick={()=>onDeleteCategoryBy(id)}><Icon name='remove' /></span>
    }
